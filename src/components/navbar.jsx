...
const Navbar = () => {
  /**
   * importing the user router functionality from next to reroute to a specific page
   * reroute uses the react hook useMemo since it is an object of functions
   * we are using useMemo for performance, since objects are call by reference not by value
   * so after every single render this object is going to be reinitialized
   * instead we need this to be initialized once, or by our definition {dependency} it is listening to
   * router update which is better than open dependencies
   */
